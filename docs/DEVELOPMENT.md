...
| 6–11 | Unit suites: parse (73), WiFi conversion (43), flows (38), capture dispatch (37), engine output (7), engine stats (5) |
| 12 | CLI integration script `tests/test_cli.sh` — 35 checks |
| 13 | Shell-completion files exist (bash, zsh, fish) |
| 14 | SDK version-check unit tests — `tests/test_sdk_check.sh`, 6 checks |

Totals on a clean run: **252 unit asserts** (groups 5–11), **35/35** CLI
